...
return :: Nat -> Tuple Nat Nat
return = pure

(>>=) :: Tuple Nat Nat -> (Nat -> Tuple Nat Nat) -> Tuple Nat Nat
Tuple u a >>= k = case k a of Tuple v b -> Tuple (u + v) b

monadAssociativity :: Tuple Nat Nat -> Nat -> (Nat -> Tuple Nat Nat) -> (Nat -> Tuple Nat Nat) -> Prop (Tuple Nat Nat)
